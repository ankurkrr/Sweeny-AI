...
interface SignupFormProps {
  onToggleMode: () => void;
  onSwitchToLogin?: (email: string, message: string) => void;
}

// onSwitchToLogin is optional for backward compatibility
export const SignupForm: React.FC<SignupFormProps> = ({ onToggleMode, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Navigate to /signin with pre-filled email and success message (no verification required)
        navigate('/signin', { state: { email, message: 'Account created! Please sign in.' } });
      } catch (err) {
        // Auto-signin failed, navigate to /signin with pre-filled email and success message
        navigate('/signin', { state: { email, message: 'Account created! Please sign in.' } });
      }
    } catch (err) {
    ...
    <div className="min-h-screen jungle-bg jungle-overlay floating-leaves flex items-center justify-center p-4 relative overflow-hidden">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-green-200/80 text-sm">Join us and start chatting with AI</p>
        </div>
